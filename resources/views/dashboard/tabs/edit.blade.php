            ...
            <div class="card shadow-lg border-0 rounded-3">
                <div class="card-header d-flex justify-content-between align-items-center py-3">
                    ...
                    <div class="d-flex gap-2">
                        <button type="button" class="btn btn-light btn-sm d-flex align-items-center" id="globalSelectAllBtn" data-bs-toggle="tooltip" data-bs-placement="top" title="Select all tabs">
                            <i class="fas fa-check-circle me-1"></i> Select All
                        </button>
                        <button type="button" class="btn btn-secondary btn-sm d-flex align-items-center" id="globalUnselectAllBtn" data-bs-toggle="tooltip" data-bs-placement="top" title="Unselect all tabs">
                                ...
                                <div class="accordion-item section-group border mb-3 rounded-3">
                                    <h2 class="accordion-header" id="heading-{{ $parent->id }}">
                                        <button class="accordion-button collapsed fw-medium" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-{{ $parent->id }}" aria-expanded="false" aria-controls="collapse-{{ $parent->id }}">
                                            <i class="{{ $parent->icon }} me-2 text-primary"></i>{{ $parent->label }}
                                            ...
                                            <div class="row row-cols-4 row-cols-sm-4 g-3">
                                                ...
                                                <div class="col">
                                                    <div class="form-check mb-2">
                                                        <input class="form-check-input tab-checkbox" type="checkbox" name="tabs[]" value="{{ $parent->id }}" id="tab-{{ $parent->id }}" aria-label="Select {{ $parent->label }} (Parent)">
                                                        <label class="form-check-label fw-semibold" for="tab-{{ $parent->id }}">
                                                            {{ $parent->label }} <small class="text-muted">(Parent)</small>
                                                        </label>
                                                    </div>
                                                </div>
                                                {{-- Children Tabs --}}
                                                @foreach ($parent->children as $child)
                                                    <div class="col">
                                                        <div class="form-check mb-2">
                                                            <input class="form-check-input tab-checkbox" type="checkbox" name="tabs[]" value="{{ $child->id }}" id="tab-{{ $child->id }}" aria-label="Select {{ $child->label }}">
                                ...
                                <div class="accordion-item section-group border mb-3 rounded-3">
                                    <h2 class="accordion-header" id="heading-no-children">
                                        <button class="accordion-button collapsed fw-medium" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-no-children" aria-expanded="false" aria-controls="collapse-no-children">
                                            <i class="fas fa-folder me-2 text-primary"></i> Tabs without children
                                        ...
                                        <div class="accordion-body">
                                            ...
                                            <div class="row row-cols-4 row-cols-sm-4 g-3">
                                                @foreach ($noChildrenTabs as $tab)
                                                    <div class="col">
                                                        <div class="form-check mb-2">
                                                            <input class="form-check-input tab-checkbox" type="checkbox" name="tabs[]" value="{{ $tab->id }}" id="tab-{{ $tab->id }}" aria-label="Select {{ $tab->label }}">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".section-select-all").on("click", function() {
                $(this).closest(".section-group").find(".tab-checkbox").prop("checked", true).trigger("change");
                $(this).addClass("animate__animated animate__pulse").one("animationend", function() {
                    $(this).removeClass("animate__animated animate__pulse");
                });
            });
            ...
            $(".section-unselect-all").on("click", function() {
                $(this).closest(".section-group").find(".tab-checkbox").prop("checked", false).trigger("change");
                $(this).addClass("animate__animated animate__pulse").one("animationend", function() {
                    $(this).removeClass("animate__animated animate__pulse");
                });
            });
