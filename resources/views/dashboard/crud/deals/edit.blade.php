...
@section('content')
            ...
            <div class="col-12 col-md-12">
                <div class="card border-0">
                    <div class="card-header">
                        <h6 class="text-lg font-weight-semibold mb-0">Edit deal</h6>
                    </div>
                    <form class="card-body" method="POST" action="{{ route('deals.update', $deal) }}">
                        @csrf
                        @method('PUT')
                        <div class="row g-3">
                            <div class="col-md-6">
                                <x-forms.input-label.basic name="name" required value="{{ old('name', $deal->name) }}">
                                    Name
                            ...
                            <div class="col-md-6">
                                <x-forms.input-label.basic
                                    name="amount"
                                    required value="{{ old('amount', $deal->amount) }}"
                                    type="number">
                                    Amount
                                </x-forms.input-label.basic>
                            </div>
                            ...
                            <div class="col-md-6">
                                <x-forms.labels.basic>Account</x-forms.labels.basic>
                                ...
                                >
                                    ...
                                    @foreach($accountsSelect as $id => $name)
                                        <option
                                            value="{{ $id }}"
                                            @selected(old('account_id', $deal->account_id) == $id)
                                        >
                                            {{ $name }}
                                        </option>
                                    @endforeach
                            ...
                            <div class="col-md-6">
                                <x-forms.labels.basic>Contact</x-forms.labels.basic>
                                <x-forms.select.basic
                                    name="contact_id"
                                    id="contactsSelect"
                                    required
                            ...
                            <div class="col-md-6">
                                <x-forms.labels.basic>Stage</x-forms.labels.basic>
                                <x-forms.select.basic name="stage" required>
                                    <option value="">Select stage</option>
                                    @foreach(StageType::cases() as $stage)
                                        <option
                                            value="{{ $stage->value }}"
                                            @selected(old('stage', $deal->stage) === $stage->value)
                                        >
                            ...
                            <div class="col-md-6">
                                <x-forms.input-label.basic
                                    ...
                                    required
                                    value="{{ old('expected_close_date', $deal->expected_close_date) }}"
                                    type="date">
                                    Expected close date
                                </x-forms.input-label.basic>
                            </div>
    ...
    <script>
        ...
        const oldAccountId = "{{ old('account_id', $deal->account_id) }}";
        const oldContactId = "{{ old('contact_id', $deal->contact_id) }}";
